...
Data::wireEncode(EncodingImpl<TAG>& encoder, bool unsignedPortion/* = false*/) const
{
  ...
  // (reverse encoding)
//-----------------------------------------------------------------------------------------------
//      THIS BLOCK WAS ADDED BY LAHMER ILYAS
//----------------------------------------------------------------------------------------------
  if(m_additionalData.size() > 0){
    const uint8_t* data = reinterpret_cast<const uint8_t*>(m_additionalData.c_str());
    ndn::EncodingEstimator estimator;
    size_t length = 0;
    length += estimator.prependByteArrayBlock(33,data,m_additionalData.length());
    length += estimator.prependVarNumber(length);
    length += estimator.prependVarNumber(33);

    ndn::EncodingBuffer buffer(length,0);
    length = 0;
    length += buffer.prependByteArrayBlock(33,data,m_additionalData.length());
    buffer.prependVarNumber(length);
    buffer.prependVarNumber(33);
    ...
    //std::cout<<"No Additional Data is Set"<<std::endl;
  }
//------------------------------------------------------------------------------------------------
...
//      THIS BLOCK WAS ADDED BY LAHMER ILYAS
//-----------------------------------------------------------------------------------------------
  val = m_wire.find(tlv::AdditionalData);
  if (val != m_wire.elements_end()) {
    //m_interestLifetime = time::milliseconds(readNonNegativeInteger(*val));
    ndn::Block block = (*val);
    block.parse();
    //std::cout<<"Getting an iterator over the block"<<std::endl;
    ndn::Block::element_const_iterator it = block.elements_begin();
    //std::cout<<"YUPPPPPPPPPPPPPPPPPY"<<std::endl;
    std::string ms = std::string(reinterpret_cast<const char*>(it->value()),it->value_size());
    //std::cout<<"FirstBlock Type :"<<it->type()<<std::endl;
    //std::cout<<"FirstBlock content "<<ms<<std::endl;
    //std::cout<<"The block was finded but ommited"<<std::endl;
    m_additionalData = ms;
  }
  else {
    //std::cout<<"The block wasn't finded"<<std::endl;
  }
//-----------------------------------------------------------------------------------------------
}
...
///     This Block was added by LAHMER ILYAS
    const std::string&
    Data::getAdditionalData() const
    {
      return m_additionalData;
    }
    Data&
    Data::setAdditionalData(const std::string& data )
    {
